...
from ingestion.dedupe import rebuild_dedup_corpus
...
DEDUP_COLLECTION_NAME = "hypertension_guidelines_dedup"
DEDUP_CHUNKS_PATH = PROCESSED_DIR / "validated_chunks_dedup.json"
DEDUP_REMAP_PATH = PROCESSED_DIR / "chunk_remap_dedup.json"
DEDUP_EMBEDDINGS_PATH = PROCESSED_DIR / "embeddings_dedup.json"
...
def sync_to_qdrant_and_storage(
    ...
    # Rebuild the deduplicated corpus + dedup Qdrant collection from the
    # accumulated validated_chunks.json (corpus-level dedup; vectors reused
    # from the original collection, no re-embedding).
    try:
        rebuild_dedup_corpus(
            chunks_path=CHUNKS_FILE_PATH,
            dedup_chunks_path=DEDUP_CHUNKS_PATH,
            remap_path=DEDUP_REMAP_PATH,
            embeddings_dedup_path=DEDUP_EMBEDDINGS_PATH,
            qdrant_path=QDRANT_PATH,
            original_collection=COLLECTION_NAME,
            dedup_collection=DEDUP_COLLECTION_NAME,
            client=client,
            vector_size=VECTOR_SIZE,
        )
    except Exception as exc:
        print(f"  [dedupe] WARNING: dedup rebuild failed: {exc}")
        print("  [dedupe] The original corpus is indexed; rerun dedupe.rebuild_dedup_corpus to fix.")
